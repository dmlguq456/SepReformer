    ...
    for i_type = 1:length(data_type)
        ...
        for i = 1:num_files
            ...
            switch min_max{i_mm}
                ...
                case 'css_v3'
                    mix_16k_length = max(length(s1_16k),length(s2_16k));
                    overlap_ratio = 0.6*rand + 0.4;
                    overlap_frames = round(mix_16k_length * overlap_ratio);
                    padding_len = mix_16k_length - overlap_frames;
                    if length(s1_16k) < length(s2_16k)
                        s1_16k = cat(1,zeros(padding_len,1), s1_16k);
                    else
                        s2_16k = cat(1,zeros(padding_len,1), s2_16k);
                    end
                    mix_16k_length = max(length(s1_16k),length(s2_16k));
                    s1_16k = cat(1,s1_16k,zeros(mix_16k_length - length(s1_16k),1));
                    s2_16k = cat(1,s2_16k,zeros(mix_16k_length - length(s2_16k),1));
